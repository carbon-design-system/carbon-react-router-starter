...
const CustomButton = forwardRef<HTMLButtonElement, CustomButtonProps>(
  ({ variant = 'primary', isLoading, children, className, disabled, ...props }, ref) => {
    return (
      <button ref={ref} className={`btn btn-${variant} ${className || ''}`} disabled={disabled || isLoading} {...props}>
...
function ListItem<T extends { id: string; name: string }>({ item, onSelect, onDelete }: ListItemProps<T>) {
  return (
    <li>
...
type CounterAction =
  { type: 'increment' } | { type: 'decrement' } | { type: 'reset' } | { type: 'setStep'; payload: number };
...
function ThemedButton() {
  ...
  return (
    <button style={{ backgroundColor: theme.primary }} onClick={toggleMode}>
...
function Select<T>({
  ...
  return (
    <select
      ...
      onChange={(e) => {
        const option = options.find((opt) => getOptionValue(opt) === e.target.value);
        if (option) {
...
function Table<T extends Record<string, unknown>>({ data, columns, keyExtractor, onRowClick }: TableProps<T>) {
...
function UsersTable({ users }: { users: User[] }) {
  const columns: Column<User>[] = [
    ...
    { key: 'email', header: 'Email' },
    {
      key: 'status',
      header: 'Status',
      render: (user) => <span className={`badge badge-${user.status}`}>{user.status}</span>,
    },
    {
      key: 'createdAt',
      header: 'Created',
...
function AuthContextProvider({ children }: { children: ReactNode }) {
  ...
  return <AuthProvider value={{ user, login, logout }}>{children}</AuthProvider>;
}
